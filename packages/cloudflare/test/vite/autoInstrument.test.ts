...
describe('sentryCloudflareAutoInstrumentPlugin', () => {
  ...
  it('prepends the orchestrion registration import to the entry', () => {
    const { transform: tx, entryPath } = createPlugin('main = "src/index.ts"');

    const code = 'export default { fetch() { return new Response("ok"); } };';
    const result = tx(code, entryPath);
    expect(result.code).toContain('import "virtual:@sentry/orchestrion-register-integrations";');
  });

  it('prepends the registration import even when the entry is already wrapped manually', () => {
    const { transform: tx, entryPath } = createPlugin('main = "src/index.ts"');
