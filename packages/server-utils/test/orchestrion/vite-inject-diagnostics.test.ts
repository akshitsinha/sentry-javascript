...
type InjectDiagnostics = (d: { failedModules: string[] }) => string;
...
describe('sentryOrchestrionPlugin — injectDiagnostics', () => {
  ...
  it('emits a self-contained banner that records the failed modules on the marker', () => {
    sentryOrchestrionPlugin({ registerIntegrations: true });
    const injectDiagnostics = captured.options?.injectDiagnostics as InjectDiagnostics;

    const banner = injectDiagnostics({ failedModules: ['mysql'] });
  ...
  it('emits an empty list when nothing failed', () => {
    sentryOrchestrionPlugin({ registerIntegrations: true });
    const injectDiagnostics = captured.options?.injectDiagnostics as InjectDiagnostics;

    // eslint-disable-next-line no-eval
    (0, eval)(injectDiagnostics({ failedModules: [] }));
  ...
  it('records failed modules on the marker and warns once at build time', () => {
    ...
    const banner = injectDiagnostics({ failedModules: ['mysql'] });
    injectDiagnostics({ failedModules: ['mysql'] });
  ...
  it('does not warn when no transform failed', () => {
    ...
    injectDiagnostics({ failedModules: [] });
